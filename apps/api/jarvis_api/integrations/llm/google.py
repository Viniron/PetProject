...
# Как выглядит отказ по стране. **Проверено живой пробой 21.09.2026**:
# `400 {"error": {"code": 400, "message": "User location is not supported
# for the API use.", "status": "FAILED_PRECONDITION"}}`. Здесь, в отличие
# от второго провайдера, формулировка говорит о географии прямо, поэтому
# опознаётся по тексту и код ответа не нужен: те же слова в теле успешного
# ответа означали бы просто текст про геолокацию, а сюда мы попадаем только
# на отказе.
