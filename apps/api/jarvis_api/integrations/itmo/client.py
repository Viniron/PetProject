...
class ItmoClient:
    ...
    def schedule(self, date_start: dt.date, date_end: dt.date, now: dt.datetime) -> SchedulePayload:
        ...
        try:
            ...
            raise ItmoFormatError(
                f"формат ответа my.itmo.ru изменился - разбор не удался:\n{ошибка}"
            ) from ошибка
    ...
    def _запросить(self, date_start: dt.date, date_end: dt.date, now: dt.datetime) -> object:
        ...
        if ответ.status_code != httpx2.codes.OK:
            raise ItmoApiError(
                f"my.itmo.ru ответил {ответ.status_code} на запрос расписания: {ответ.text[:200]!r}"
            )
        ...
        self._проверить_код(тело)
        return тело

    @staticmethod
    def _проверить_код(тело: object) -> None:
        """Портал сообщает об отказе внутри тела, а не кодом HTTP.

        Найдено первым живым прогоном: на успехе ответ содержит `code: 0`
        и `message: null` рядом с `data`. Значит отказ вида
        `{"code": 500, "message": "...", "data": []}` приходит с HTTP 200 -
        и без этой проверки выглядит как пустое расписание.

        Цена ошибки здесь наибольшая во всём этапе. Окно зеркала заменяется
        целиком, поэтому «пустое расписание» стирает все пары окна, а Э4
        следом удалит их из Google. Каникулы и отказ сервиса обязаны
        различаться, и различает их только это поле.

        Отсутствие `code` считается успехом намеренно: поле неофициальное,
        как и вся схема, и его исчезновение не повод отказываться от
        расписания, которое портал всё-таки отдал.
        """
        if not isinstance(тело, dict):
            return
        код = тело.get("code")
        if код is None or код == 0:
            return
        сообщение = тело.get("message")
        raise ItmoApiError(
            f"my.itmo.ru сообщил об отказе внутри ответа: code={код!r}, "
            f"message={сообщение!r} (HTTP {httpx2.codes.OK} при этом получен)"
        )
