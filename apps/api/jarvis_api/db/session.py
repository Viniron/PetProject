...
def get_engine() -> Engine:
    ...
    return create_engine(
        ...
        pool_size=5,
        max_overflow=5,
        # Оба таймаута появились с эндпоинтами (Э6) и оба про инвариант 9:
        # интерфейс обязан деградировать, а не показывать вечный спиннер.
        # Без connect_timeout запрос к недоступной базе висит до таймаута
        # TCP операционной системы, занимая поток из пула; pool_pre_ping
        # не помогает - сам ping зависает так же. Без pool_timeout запрос,
        # пришедший во время прогона цепочки, ждёт свободное соединение
        # 30 секунд по умолчанию - это тот же спиннер, только изнутри.
        connect_args={"connect_timeout": настройки.db_connect_timeout_seconds},
        pool_timeout=настройки.db_pool_timeout_seconds,
    )
