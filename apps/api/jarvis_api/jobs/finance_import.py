...
class ОтчётФайла:
    ...
    # Сам план - для тех, кому мало счётчиков. Команда печатает числа,
    # а экран импорта (Ф6, §15.7) показывает строки: это единственная точка,
    # где owner видит, что попадёт в книжку, до того как оно туда попадёт.
    план: ПланИмпорта | None = None
