...
и недельный бюджет с дневным лимитом (Ф13, §15.10).
...
# --- недельный бюджет (Ф13, §15.10) -----------------------------------------
#
# Шесть ручек, и все считают одно и то же одним доменом. Арифметика лимита
# сюда не переезжает ни в каком виде: цифра, посчитанная на клиенте или
# в роутере вторично, разошлась бы с той, по которой owner тратит деньги
# (инвариант 1).

ОПИСАНИЕ_НЕДЕЛИ = "Понедельник недели, YYYY-MM-DD. Не задан - текущая неделя в зоне owner"


def _сегодня(момент: dt.datetime, зона: ZoneInfo) -> dt.date:
    """«Сегодня» owner, а не UTC: от него считаются открытые дни недели."""
    return момент.astimezone(зона).date()


def _отказ_бюджета(сбой: finance_budget.ОшибкаБюджета) -> ОтказAPI:
    """Код ответа по коду отказа.

    422 - запрос неверен сам по себе (вторник вместо понедельника,
    отрицательные деньги); 404 - строки нет; 409 - запрос законный,
    но противоречит состоянию книжки, и это owner обязан прочитать.
    """
    неверный_запрос = ("не_понедельник", "бюджет_отрицателен", "трата_отрицательна")
    if сбой.код in неверный_запрос:
        статус = 422
    elif сбой.код == "нет_дня":
        статус = 404
    else:
        статус = 409
    return ОтказAPI(статус=статус, code=сбой.код, message=str(сбой))
...
def _неделя_наружу(сводка: finance_budget.Сводка) -> BudgetWeekOut:
    """Неделя домена в модель контракта.

    Собирается в одном месте и для экрана недели, и для списка недель:
    две сборки разошлись бы в том, что считается «потрачено», - а это
    та самая цифра, по которой owner решает, сколько тратить завтра.
    """
    неделя = сводка.неделя
    распределение = None
    if (
        сводка.распределена_в is not None
        and сводка.в_следующую is not None
        and сводка.в_копилку is not None
        and сводка.снимок_трат is not None
    ):
        распределение = SettlementOut(
            to_next=сводка.в_следующую,
            to_savings=сводка.в_копилку,
            settled_spend=сводка.снимок_трат,
            settled_at=сводка.распределена_в,
        )

    return BudgetWeekOut(
        week_start=неделя.начало,
        week_end=неделя.конец,
        budget=неделя.бюджет,
        carry=неделя.перенос,
        spent=неделя.потрачено,
        settled=неделя.распределена,
        days=[BudgetDayOut.model_validate(день) for день in неделя.дни],
        limit=LimitOut.model_validate(сводка.лимит_),
        ready_to_settle=сводка.итог_ is not None,
        outcome=OutcomeOut.model_validate(сводка.итог_) if сводка.итог_ is not None else None,
        settlement=распределение,
    )


def _неделя(значение: dt.date | None, момент: dt.datetime, зона: ZoneInfo) -> dt.date:
    """Запрошенная неделя, а без параметра - текущая."""
    if значение is None:
        return finance_budget.начало_недели(_сегодня(момент, зона))
    return значение


def _ответ_дня(
    сессия: Session, запись: finance_budget.ЗаписьДня, сегодня: dt.date, зона: ZoneInfo
) -> DaySpendOut:
    """Что стало с днём - вместе с пересчитанной неделей.

    Неделя отдаётся не из вежливости: внесённая сумма закрывает день, то есть
    меняет делитель, и лимит после ввода означает уже «завтра». Считать это
    второй раз на клиенте значило бы завести вторую арифметику денег.
    """
    начало = finance_budget.начало_недели(запись.день)
    return DaySpendOut(
        day=запись.день,
        amount=запись.сумма,
        previous=запись.было,
        corrected_weeks=list(запись.поправлены),
        week=_неделя_наружу(finance_budget.сводка(сессия, начало, сегодня=сегодня, зона=зона)),
    )


@маршрутизатор.get("/budget", responses=ОТКАЗЫ, summary="Неделя бюджета и дневной лимит")
def бюджет(
    сессия: Сессия,
    момент: Сейчас,
    week: Annotated[dt.date | None, Query(description=ОПИСАНИЕ_НЕДЕЛИ)] = None,
) -> BudgetOut:
    """Сколько можно потратить и в какой день (§15.10, экран 15 дизайна).
    ...
    """
    зона = _зона(сессия)
    сегодня = _сегодня(момент, зона)
    try:
        сводка = finance_budget.сводка(
            сессия, _неделя(week, момент, зона), сегодня=сегодня, зона=зона
        )
    except finance_budget.ОшибкаБюджета as сбой:
        raise _отказ_бюджета(сбой) from сбой

    return BudgetOut(
        week=_неделя_наружу(сводка),
        today=сегодня,
        timezone=str(зона),
        covered_through=finance_budget.покрыто_по(сессия),
        savings_jar=finance_budget.копилка(сессия),
        saved_real=отложено_всего(сессия, зона=зона),
    )


@маршрутизатор.get("/budget/weeks", responses=ОТКАЗЫ, summary="Недели подряд")
def недели_бюджета(
    сессия: Сессия,
    момент: Сейчас,
    начало: Annotated[dt.date | None, Query(alias="from", description=ОПИСАНИЕ_НЕДЕЛИ)] = None,
    weeks: Annotated[int, Query(ge=1, le=26, description="Сколько недель отдать подряд")] = 5,
) -> BudgetWeeksOut:
    """Список для ввода бюджета вперёд: owner вносит его обычно на месяц.
    ...
    зона = _зона(сессия)
    сегодня = _сегодня(момент, зона)
    с_недели = _неделя(начало, момент, зона)
    try:
        сводки = finance_budget.сводки(
            сессия, с=с_недели, сколько=weeks, сегодня=сегодня, зона=зона
        )
    except finance_budget.ОшибкаБюджета as сбой:
        raise _отказ_бюджета(сбой) from сбой

    return BudgetWeeksOut(from_week=с_недели, weeks=[_неделя_наружу(сводка) for сводка in сводки])


@маршрутизатор.put(
    "/budget/weeks/{week_start}",
    responses=ОТКАЗЫ | КОНФЛИКТ,
    summary="Внести бюджет недели",
)
def внести_бюджет_недели(
    сессия: Сессия, момент: Сейчас, week_start: dt.date, тело: WeekBudgetIn
) -> BudgetWeekOut:
    """Бюджет на неделю вперёд или правка внесённого (§15.10).
    ...
    сегодня = _сегодня(момент, зона)
    try:
        finance_budget.внести_бюджет(сессия, начало=week_start, сумма=тело.amount)
    except finance_budget.ОшибкаБюджета as сбой:
        raise _отказ_бюджета(сбой) from сбой

    сессия.commit()
    return _неделя_наружу(finance_budget.сводка(сессия, week_start, сегодня=сегодня, зона=зона))


@маршрутизатор.post(
    "/budget/weeks/{week_start}/settle",
    responses=ОТКАЗЫ | КОНФЛИКТ,
    summary="Распределить итог недели",
)
def распределить_итог(
    сессия: Сессия, момент: Сейчас, week_start: dt.date, тело: SettleIn
) -> BudgetWeekOut:
    """Излишек или перерасход - в следующую неделю и в копилку (§15.10).
    ...
    сегодня = _сегодня(момент, зона)
    try:
        сводка = finance_budget.распределить(
            сессия,
            начало=week_start,
            в_следующую=тело.to_next,
            в_копилку=тело.to_savings,
            сегодня=сегодня,
            сейчас=момент,
            зона=зона,
        )
    ...
    сессия.commit()
    return _неделя_наружу(сводка)


@маршрутизатор.put(
    "/budget/days/{day}",
    responses=ОТКАЗЫ | КОНФЛИКТ,
    summary="Внести трату за день",
)
def внести_трату_дня(сессия: Сессия, момент: Сейчас, day: dt.date, тело: DaySpendIn) -> DaySpendOut:
    """Вечерний ввод owner - то, чем книжка живёт до приезда выписки (§15.10).
    ...
    сегодня = _сегодня(момент, зона)
    try:
        запись = finance_budget.внести_день(
            сессия,
            день=day,
            сумма=тело.amount,
            заметка=тело.note,
    ...
    сессия.commit()
    return _ответ_дня(сессия, запись, сегодня, зона)


@маршрутизатор.delete(
    "/budget/days/{day}",
    responses=ОТКАЗЫ | НЕ_НАЙДЕНО,
    summary="Снять внесённую трату за день",
)
def снять_трату_дня(сессия: Сессия, момент: Сейчас, day: dt.date) -> DaySpendOut:
    """Возврат дня в «неизвестно» - единственный способ отменить ошибочный ввод.
    ...
    сегодня = _сегодня(момент, зона)
    try:
        запись = finance_budget.снять_день(сессия, день=day, сегодня=сегодня, зона=зона)
    except finance_budget.ОшибкаБюджета as сбой:
        raise _отказ_бюджета(сбой) from сбой
