...
def портал() -> ПодставнойПортал:
    return ПодставнойПортал(загрузить_фикстуру("itmo", "schedule_live.json"))
...
def test_apply_записывает_зеркало(база_с_настройками: Session, http: httpx2.Client) -> None:
    """Шесть пар живой фикстуры ложатся в базу с местной датой и временем в UTC."""
    код = run_once(база_с_настройками, настройки(), http, apply=True, now=СЕЙЧАС)
    строки = пары(база_с_настройками)

    assert код == 0
    assert len(строки) == 6
    assert строки[0].subject == "Встреча на факультете/ОП"
...
def test_исчезнувшая_из_расписания_пара_исчезает_из_зеркала(
    ...
    урезанное = загрузить_фикстуру("itmo", "schedule_live.json")
    урезанное["data"][0]["lessons"] = урезанное["data"][0]["lessons"][:1]
    портал.расписание = урезанное
    run_once(база_с_настройками, настройки(), http, apply=True, now=СЕЙЧАС)

    оставшиеся = [строка.subject for строка in пары(база_с_настройками)]
    assert len(оставшиеся) == 5
    assert "Распределительный тест" not in оставшиеся
    assert оставшиеся[0] == "Встреча на факультете/ОП"
...
def test_изменившаяся_пара_обновляется_а_не_добавляется(
    ...
    изменённое = загрузить_фикстуру("itmo", "schedule_live.json")
...
def test_успешный_забор_оставляет_след_в_журналах(
    ...
    assert запись.detail is not None and запись.detail["added"] == 6
...
def test_без_строки_настроек_берётся_московская_зона(сессия: Session, http: httpx2.Client) -> None:
    ...
    assert код == 0
    assert пары(сессия)[0].starts_at == dt.datetime(2026, 8, 30, 7, 0, tzinfo=dt.UTC)


# --- отказ портала под видом успеха -----------------------------------------


def test_ошибка_в_теле_ответа_не_стирает_зеркало(
    база_с_настройками: Session, http: httpx2.Client, портал: ПодставнойПортал
) -> None:
    """Самый дорогой отказ этапа, найденный первым живым прогоном.

    Портал сообщает об отказе внутри тела: `code != 0` при HTTP 200.
    Окно зеркала заменяется целиком, поэтому без проверки `code` такой
    ответ выглядел бы как каникулы и стёр бы все пары окна — а Э4 следом
    удалил бы их из Google.
    """
    run_once(база_с_настройками, настройки(), http, apply=True, now=СЕЙЧАС)
    assert len(пары(база_с_настройками)) == 6

    портал.расписание = загрузить_фикстуру("itmo", "schedule_error_code.json")
    код = run_once(база_с_настройками, настройки(), http, apply=True, now=СЕЙЧАС)

    assert код == 1
    assert len(пары(база_с_настройками)) == 6
    запись = база_с_настройками.scalars(
        select(AuditLogEntry).order_by(AuditLogEntry.id.desc())
    ).first()
    assert запись is not None
    assert "сообщил об отказе внутри ответа" in str(запись.detail["error"] if запись.detail else "")


def test_каникулы_от_отказа_отличаются(
    база_с_настройками: Session, http: httpx2.Client, портал: ПодставнойПортал
) -> None:
    """Обратная сторона предыдущего теста: пустое расписание с `code: 0` —
    это каникулы, и зеркало обязано опустеть.

    Оба случая приходят с HTTP 200 и пустым `data`, различает их только
    поле `code`. Без этой пары тестов легко «починить» одно, сломав другое.
    """
    run_once(база_с_настройками, настройки(), http, apply=True, now=СЕЙЧАС)
    портал.расписание = {"code": 0, "message": None, "data": []}
...
def test_ответ_без_поля_code_считается_успехом(
    база_с_настройками: Session, http: httpx2.Client, портал: ПодставнойПортал
) -> None:
    """Поле неофициальное, как и вся схема портала.

    Его исчезновение — не повод отказываться от расписания, которое портал
    всё-таки отдал: это привело бы к отказу от работы там, где данные есть.
    """
    портал.расписание = {"data": загрузить_фикстуру("itmo", "schedule_live.json")["data"]}
...
def test_отказ_в_теле_повторяется(
    база_с_настройками: Session, http: httpx2.Client, портал: ПодставнойПортал
) -> None:
    """`code != 0` — отказ сервиса, а не смена формата: повтор уместен.

    Отличие от нечитаемого ответа существенно: тот же запрос через минуту
    вполне может пройти, если у портала была разовая внутренняя ошибка.
    """
    портал.расписание = загрузить_фикстуру("itmo", "schedule_error_code.json")
