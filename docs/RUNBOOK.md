...
4. Public Hostname: поддомен (например `jarvis`), домен, service
   `http://web:8080` — туннель и фронт в одной сети compose, поэтому
   обращение по имени сервиса. Именно `web`, а не `api`: Caddy раздаёт
   интерфейс и он же проксирует `/api`, а прямой маршрут на API выставил бы
   наружу голый API без единой страницы.
...
Строк за день три: `daily_calendar` — итог цепочки, `sync_itmo` и `push_gcal` —
её шаги. Строка `running` с пустым `finished_at` означает, что процесс убили
